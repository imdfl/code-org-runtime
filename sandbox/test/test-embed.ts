...
$(() => {
	const $root = $("#sandbox");
	const sceneWidth = $root.width() || 400,
		sceneHeight = $root.height() || 400;

	const scene: ISJSScene = sjs.Scene({
		w: sceneWidth,
		h: sceneHeight,
		parent: $root[0]
	});
	...
	const bg: SpriteWrapper = new SpriteWrapper(0, 0);
	bg.width = sceneWidth;
	...
	var PLAYING = 2,
		...
		gameState = OPENING,
		bouns = false,
		ndnPowerUpTypes = ["doubleShot", "shield"],
		ndndActivePowerUps = [],
		startButton = false,
		openingDone,
		doubleShot = false,

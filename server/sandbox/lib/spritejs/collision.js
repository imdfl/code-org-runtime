...
(function (global) {
	...
	function rectangleEdges(rect) {
		...
		var distance = sjs.math.hypo(rect.effectiveWidth / 2, rect.effectiveHeight / 2);
		var angle = Math.atan2(rect.effectiveHeight, rect.effectiveWidth);
		...
		for (var i = 0; i < 4; i++) {
			points.push([
				distance * Math.cos(rect.angle + angles[i]) + rect.x + rect.effectiveWidth / 2,
				distance * Math.sin(rect.angle + angles[i]) + rect.y + rect.effectiveHeight / 2
			]);
		}
		return points;
	};
	...
	function shapeCenter(s) {
		return {
			x: (s.x + s.effectiveWidth / 2),
			y: (s.y + s.effectiveHeight / 2)
		};
	}
	...
	function makeBoundingBox(a) {
		...
		return {
			...
			w: a.shape.w,
			h: a.shape.h,
			effectiveWidth: a.shape.w * a.xscale,
			effectiveHeight: a.shape.h * a.yscale,
			angle: a.angle,
	...
	function shapeCollides(a, b) {
		...
		// quick shortcut
		if (Math.abs(a.x - b.x) > a.effectiveWidth + b.effectiveWidth) {
			return false;
		}

		if (Math.abs(a.y - b.y) > a.effectiveHeight + b.effectiveHeight) {
			return false;
		}
	...
	function rectangleCollides(a, b) {
		...
		if (a.angle == 0 && b.angle == 0) {
			if (a.x > b.x) {
				x_inter = a.x - b.x < b.effectiveWidth;
			} else {
				x_inter = b.x - a.x < a.effectiveWidth;
			}
			if (x_inter == false)
				return false;

			if (a.y > b.y) {
				y_inter = a.y - b.y < b.effectiveHeight;
			} else {
				y_inter = b.y - a.y < a.effectiveHeight;
			}
			return y_inter;
		};

		// cache the expensive edges function
		a.object.edges = rectangleEdges(a);
		b.object.edges = rectangleEdges(b);
	...
	function circleCollides(a, b) {
		...
		return sjs.math.hypo(c1.x - c2.x, c1.y - c2.y) < a.effectiveWidth / 2 + b.effectiveWidth / 2;
	}
	...
	function isPointInRectangle(x, y, rect) {
		...
		if (rect.angle == 0 || rect.angle === undefined)
			return (x >= rect.x && x < rect.x + rect.effectiveWidth
				&& y >= rect.y && y < rect.y + rect.effectiveHeight);
